...
public class GraphTest extends BaseTest
{
     ...
     */
    @Test
    @Description("Verifying Graph page")
    @Epic("09")
    @Feature("Feature1:Graph page")
    @Story("Story:Checking Graph page")
    @Step("Graph page")
    @Severity(SeverityLevel.MINOR)
    public void graph()
    {
        ExtentTest extentTest = extent.createTest("Graph page").log(Status.PASS, "Graph page");
        extentTest.log(Status.PASS,"Graph test is passed");
        ...
        graphPage.graphLinkOnGraphPage();
        graphPage.graphLinkOnGraphPage();
        linkedList.tryHereClick();
        String pythonCode = FileManager.ReadCellData(3, 1);
        linkedList.getEnteringCode().sendKeys(pythonCode);
        linkedList.run();
        driver.navigate().back();
        graphPage.graphRepreLink();
        linkedList.tryHereClick();
        linkedList.getEnteringCode().sendKeys(pythonCode);
        linkedList.run();
        driver.navigate().back();
        graphPage.practiceQuetions();
